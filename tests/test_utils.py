...
class URLFetcherTest(SimpleTestCase):
    def setUp(self):
        ...
        self.fetcher = DjangoURLFetcher()

    def test_default(self):
        # MEDIA_URL='' and STATIC_URL=None, all requests passed though
        url = 'https://s3.amazon.test/images/image.jpg'
        with mock.patch('weasyprint.urls.URLFetcher.fetch') as url_fetcher:
            self.fetcher.fetch(url)
        url_fetcher.assert_called_once_with(url, None)
    ...
    def assert_data(self, response, file_path, mime_type):
        self.assertEqual(response.url, 'file://' + file_path)
        self.assertEqual(response.content_type, mime_type)
    ...
    @mock.patch('weasyprint.urls.URLFetcher.fetch')
    def test_media_with_trailing_slash(self, mock_fetcher, mock_open):
        ...
        self.assert_data(response, '/www/media/image.jpg', 'image/jpeg')
    ...
    def test_static(self, mock_fetcher, mock_find, mock_open):
        ...
        self.assert_data(response, '/www/static/css/styles.css', 'text/css')
    ...
    def test_manifest_static(self, mock_fetcher, mock_find, mock_open, hashed_files):
        ...
        # fetcher is still able to find the file.
        with override_settings(DEBUG=True):
            response = self.fetcher.fetch('file:///static/css/styles.css')
        mock_fetcher.assert_not_called()
        mock_find.assert_called_once_with('css/styles.css')
        mock_open.assert_called_once_with('/www/static/css/styles.css', 'rb')
        self.assert_data(response, '/www/static/css/styles.css', 'text/css')
    ...
    def test_static_file_not_found(self, mock_fetcher, mock_find, mock_open):
        ...
        mock_fetcher.assert_called_once_with('file:///static/css/missing.css', None)
